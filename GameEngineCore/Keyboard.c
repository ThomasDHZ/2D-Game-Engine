...
static void UpdateKeyState(const SDL_Event* event, KeyState* keyState)
{
    if (keyState == KS_Pressed ||
        keyState == KS_Held)
    {
        if (global.Keyboard.KeyPressed[ConvertToAscii(event->key.keysym.sym)] == KS_Pressed)
        {
            global.Keyboard.KeyPressed[ConvertToAscii(event->key.keysym.sym)] = KS_Held;
        }
        else
        {
            global.Keyboard.KeyPressed[ConvertToAscii(event->key.keysym.sym)] = KS_Pressed;
        }
    }
    else
    {
        global.Keyboard.KeyPressed[ConvertToAscii(event->key.keysym.sym)] = KS_Unpressed;
    }
}

void GameEngine_KeyboardKeyDown(const SDL_Event* event)
{
    UpdateKeyState(event, KS_Pressed);
}

void GameEngine_KeyboardKeyUp(const SDL_Event* event)
{
    UpdateKeyState(event, KS_Unpressed);
}
