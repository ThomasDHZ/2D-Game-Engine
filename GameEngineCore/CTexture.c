...
{
	if (textureInfo->MipMapLevels > 1)
	{
		int32_t mipWidth = *textureInfo->Width;
		int32_t mipHeight = *textureInfo->Height;

		VkFormatProperties formatProperties;
		vkGetPhysicalDeviceFormatProperties(global.Renderer.PhysicalDevice, *textureInfo->TextureByteFormat, &formatProperties);
...
{
	return vkCreateImageView(global.Renderer.Device, imageViewCreateInfo, NULL, textureInfo->View);
}

VkResult Texture_CreateTextureSampler(struct TextureInfo* textureInfo, VkSamplerCreateInfo* samplerCreateInfo)
{
	return vkCreateSampler(global.Renderer.Device, samplerCreateInfo, NULL, textureInfo->Sampler);
}
